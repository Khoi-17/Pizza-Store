...
public class CreateCategoryController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            CategoryDTO categoryDTO =new CategoryDTO(Long.parseLong(categoryId), categoryname, description);
            if(categoryDAO.addCategory(categoryDTO)){
                response.sendRedirect("CategoryManagementController");
            }else{
                request.setAttribute("message", "Create Category failed");
            request.getRequestDispatcher("create-category.jsp").forward(request, response);
            }
        }catch (Exception e){
            log("Somethinge Went Wrong");
        }
    }
